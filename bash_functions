...
function ex {
  if [ -f $1 ] ; then
    case $1 in
      ...
      *.deb)                  ar x $1           ;;
      *.gz)                   gunzip $1         ;;
      *.lzip)                 tar -vkxf --lzip $1 ;;
      *.lzma)                 tar -vkxf --lzma $1 ;;
      *.lzop)                 tar -vkxf --lzop $1 ;;
      *.rar)                  unrar x $1        ;;
      *.tar)                  tar -vkxf $1        ;;
      *.tar.bz2|*.tbz2|*.tb2) tar -vkxjf $1       ;;
      *.tar.bz|*.tbz)         tar -vkxjf $1       ;;
      *.tar.gz|*.tgz)         tar -vkxzf $1       ;;
      *.tar.xz|*.txz)         tar -vkxJf $1       ;;
      *.tar.zst)              tar -vkxf $1        ;;
      *.Z)                    uncompress $1     ;;
      *.zip)                  unzip $1          ;;
